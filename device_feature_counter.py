        ...
        if device_type == 'ALFO80HDX':
            ...
            for ne in root.findall('.//AGS20M'):
                ...
                for up in ne.findall('.//UpperProfile'):
                    if up.text:
                        if '1024' in up.text or '2048' in up.text:
                            count_1024_2048 += 1
                        if '4096' in up.text:
                            count_4096 += 1
                ...
                count_dual = sum(1 for odu in ne.findall('.//OduPartNumber') if odu.text and odu.text.startswith('GF'))
                feature_count['Dual Carrier'] = count_dual

                count_mbl = sum(1 for port in ne.findall('.//RadioAggregationMemberPort') if port.text and 'ALFO' in port.text)
                feature_count['MBL'] = count_mbl

                count_112 = sum(1 for band in ne.findall('.//BandAndMod') if band.text and band.text.startswith('112'))
